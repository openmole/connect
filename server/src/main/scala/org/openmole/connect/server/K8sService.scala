  ...
  def toPodInfo(pod: Pod) =
    ...
    val st =
      containerStatus.flatMap(_.state).map:
        case Container.Waiting(reason) => Data.PodInfo.Status.Waiting(reason.getOrElse(""))
        case _: Container.Running => Data.PodInfo.Status.Running
        case Container.Terminated(_, _, _, message, _, finishedAt, _) =>
          Data.PodInfo.Status.Terminated(message.getOrElse(""), finishedAt.map { _.toEpochSecond }.getOrElse(0L))
