...
object K8sService {
  ...
  def toPodInfoList(pod: Pod) =
    for {
      ...
      val st: Data.Status = state match {
      ...
      PodInfo(pod.name, st.value, restarts.restartCount, createTime.toEpochSecond, podIP, DB.email(pod.metadata.labels.getOrElse("podName", "")).map {
        _.value
      })
    }
